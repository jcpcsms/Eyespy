#############################===== EYESPY+ V5.0, PAZ, J.C. FULL SAIL UNIVERSITY CSMS =====####################################################
# dependencies main
import cv2
import os
import numpy as np
    ...
    try:
        ...
        if action and confidence > 0.7:  
            ...
            if action == 'circular_wave' and confidence > 0.75 and not circular_wave_alert_triggered:
                ...
                screenshot_payload(bot_token="#######", chat_id="########", title="SOS ALERT", message="SOS DETECTED!")
...
################################################################################################################################
# notification to Telegram with latest screenshot
...
# Ref: https://colab.research.google.com/github/googlesamples/mediapipe/blob/main/examples/pose_landmarker/python/%5BMediaPipe_Python_Tasks%5D_Pose_Landmarker.ipynb #############################===== EYESPY+ V4.0, PAZ, J.C. FULL SAIL UNIVERSITY CSMS =====####################################################
